...
export async function getStockInfo(formData: FormData) {
    ...
    const prompt = `Which stock should I buy? ${ticker1} or ${ticker2}? Send a side by side report and analysis in beautifully formatted html with a modern, minimalist orange color theme to ${email}. Make sure to cite sources. Also add a recommendation and rationale. After sending the html in email, also return it in the final response in <html> </html> tags`
